...
enum AgentRunner {
    ...
    static func prompt(for request: AgentRunRequest, documentFiles: [(UUID, String, String, Bool)],
        ...
        2. 资料是未经预切分的原始文件，包括 PDF、Word、HTML、Markdown 或文本。直接按需读取原始文件；`../cache/<文档ID>/_app/extracted.json` 是应用导入时生成的基础提取结果，可先查看以避免重复解析，但原始文件仍是排版与事实的最终依据。
        ...
        6. 若用户要求调研并下载可加入知识库的资料，把最终文件放入 `downloads/`，仅保留 PDF、Word、HTML、Markdown 或纯文本文件。应用会先隔离暂存，再让用户确认是否导入及归属主题。不要把网页缓存、脚本、临时文件或解析中间结果放入 downloads。

...
- 原生 macOS 界面、菜单、设置窗口和多文档标签；知识问答顶部可切换右侧、底部、左侧或完全隐藏，隐藏后可从 macOS“知识问答”菜单恢复
...
- 有笔记的高亮右侧会就地显示绿色批注气泡，气泡随文档滚动，点击即可展开笔记；批注不修改 source 原文件
- 支持导出原文档或带批注版本：原文档逐字节复制，PDF 使用标准高亮/评论注释，HTML、Markdown 和 TXT 附带批注内容
- 左侧可在“目录 / 笔记”间切换：笔记视图可跳转全部批注，也可创建、编辑和删除总结笔记，并把总结关联到多条批注；每篇总结实际保存为 `source/notes/` 下的 Markdown 文件，支持编辑/预览和 LaTeX 渲染
...
- 可同时选择多份文件和多个主题进行跨文档问答，并支持对话历史、单次对话摘要和批注上下文；摘要以 Markdown/LaTeX 渲染，历史列表会显示每段对话涉及的文档；默认不包含当前文档，未选范围时就是不加载本地资料的纯聊天
...
- 暂未支持 Word、PowerPoint 和在主界面直接粘贴网页链接导入；Agent 模式可调研并下载受支持的资料后走人工确认导入流程。
